...
if __name__ == '__main__':
    ...
    for desc in df['desc'].tolist()[:10]:
        word_lst = jieba.cut(desc)
        words = [re.sub('\s', '', w).lower() for w in word_lst if re.sub('\s', '', w) and len(w) > 1 and not re.findall(r'[0-9a-zA-Z]+', w)]
        ret = Counter(words)
        for k, v in ret.most_common(10):
